...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        global last_box_num, pred_dicts_sot, init_h, temp_, temp_pc
        # start = time.time()
        ...
        if selected_id == None or selected_id == -1:
            ...
            if np.where(pred_dicts_sot[0]['pred_id'] == selected_id)[0].shape[0] == 0:
                ...
                if slc_label == 1:
                    ...
                    if ocl_count < args.max_occlusion:
                        try:
                            ...
                            dist_mask_c, dist_val_c =  get_target_distance_v3(sot_list[-1].copy(), bbox_list)
                            dist_mask_d, dist_val_d =  get_target_distance_v3(sot_list[-1].copy(), pred_dicts[0]['pred_boxes'].copy())
                            
                            if dist_val_c[0].item() < dist_val_d[0][dist_mask_d].item():
                                # import pdb; pdb.set_trace()
                                temp_ = [{}]
                                temp_[0]['pred_boxes'] = bbox_list[dist_mask_c]
                                temp_[0]['pred_scores'] = np.ones(bbox_list.shape[0])[dist_mask_c]
                                temp_[0]['pred_labels'] = np.full(bbox_list.shape[0], slc_label)[dist_mask_c]
                                ...
                                # import pdb; pdb.set_trace()
                                temp_ = [{}]
                                temp_[0]['pred_boxes'] = pred_dicts[0]['pred_boxes'][dist_mask_d]
                                temp_[0]['pred_boxes'][:, 2] = prev_z
                                temp_[0]['pred_scores'] = np.ones(pred_dicts[0]['pred_boxes'].shape[0])[dist_mask_d]
                                temp_[0]['pred_labels'] = np.full(pred_dicts[0]['pred_boxes'].shape[0], slc_label)[dist_mask_d]
