...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None or selected_id == -1:
            if pred_dicts[0]['pred_labels'].shape[0] != 0:
                ...
                try:
                    ...
                    # img_stack = np.vstack([ref_img[np.newaxis,...],range_img[np.newaxis,...],ref_img[np.newaxis,...]])
                    # img_stack = cv2.applyColorMap(img_stack.transpose(1,2,0), cv2.COLORMAP_COOL)
                    # import pdb; pdb.set_trace()
                    # torch.cuda.synchronize()
                    # self.starter.record()
                    ...
                    box3d = _pred_dicts_[0]['pred_boxes']
                    box3d_cls = _pred_dicts_[0]['pred_labels'].reshape(-1, 1) # (class_id) # yolo_class: 1:ped, 3:car, 6:bus, 8:truck, -> 0, 2, 5, 7
                    ...
                    if best_M_sorted.shape[0] > 0: #2D-3D fusion
                        pub_dicts[0]['pred_boxes'][:_pred_dicts_[0]['pred_boxes'][best_M_sorted][:args.num_candidate, :].shape[0], :] = _pred_dicts_[0]['pred_boxes'][best_M_sorted][:args.num_candidate, :]
                        pub_dicts[0]['pred_scores'][:_pred_dicts_[0]['pred_scores'][best_M_sorted][:args.num_candidate].shape[0]] = _pred_dicts_[0]['pred_scores'][best_M_sorted][:args.num_candidate]
                        pub_dicts[0]['pred_labels'][:_pred_dicts_[0]['pred_labels'][best_M_sorted][:args.num_candidate].shape[0]] = _pred_dicts_[0]['pred_labels'][best_M_sorted][:args.num_candidate]
                        pub_dicts[0]['pred_id'][:_pred_dicts_[0]['pred_id'][best_M_sorted][:args.num_candidate].shape[0]] = _pred_dicts_[0]['pred_id'][best_M_sorted][:args.num_candidate]
                    else: # no 2D results case
                        pub_dicts[0]['pred_boxes'][:_pred_dicts_[0]['pred_boxes'][:args.num_candidate, :].shape[0], :] = _pred_dicts_[0]['pred_boxes'][:args.num_candidate, :]
                        pub_dicts[0]['pred_scores'][:_pred_dicts_[0]['pred_scores'][:args.num_candidate].shape[0]]  = _pred_dicts_[0]['pred_scores'][:args.num_candidate]
                        pub_dicts[0]['pred_labels'][:_pred_dicts_[0]['pred_labels'][:args.num_candidate].shape[0]]  = _pred_dicts_[0]['pred_labels'][:args.num_candidate]
                        pub_dicts[0]['pred_id'][:_pred_dicts_[0]['pred_id'][:args.num_candidate].shape[0]]  = _pred_dicts_[0]['pred_id'][:args.num_candidate]
            ...
            if invalid_skip_flag:
                ...
                if slc_label == 1:
                    ...
                    if ocl_count >= args.max_occlusion:
                        ...
                        label_mask = (pred_dicts_mot[0]['pred_labels']==prev_label)
                        if label_mask.sum() != 0: # no same cls obj in current detection result 
                            pred_dicts[0]['pred_boxes'] = pred_dicts_mot[0]['pred_boxes'][label_mask]
                            pred_dicts[0]['pred_scores'] = pred_dicts_mot[0]['pred_scores'][label_mask]
                            pred_dicts[0]['pred_labels'] = pred_dicts_mot[0]['pred_labels'][label_mask]
                            pred_dicts[0]['pred_id'] = pred_dicts_mot[0]['pred_id'][label_mask]
                    ...
                    if len(sot_list) > 1:
                        ...
                        heading_average = np.mean(heading_list)
                        speed = np.sqrt(np.array(vx**2+vy**2))
                        heading_vx = speed * np.cos(heading_average)
                        heading_vy = speed * np.sin(heading_average)
                        vx_dif = np.round(abs(vx - heading_vx), 2)
                        vy_dif = np.round(abs(vy - heading_vy), 2)
                        # if (vx_dif > 0.3) or (vy_dif > 0.3):
                        #     print('vx, vy difference: ', vx_dif, vy_dif)
                        ...
                        # x += vx * (1/obj_dist) * 2
                        # y += vy * (1/obj_dist) * 2
                        x += heading_vx * (1/obj_dist) * 2
                        y += heading_vy * (1/obj_dist) * 2
                        print('dist test: ', 1/obj_dist, (np.exp(1/obj_dist)/4))
                        # print('1/obj_dist * 2:',(np.round(1/obj_dist, 2) * 2))

                        # vx_list = []
                        # v_list.append()
                        # if abs(vx) < 0.07 and abs(vy) < 0.07:
                        ...
                        # vy_list.append(vy)
                        # count = 0
                        # for vy_val in vy_list:
                        #     if abs(vy_val) > 0.3:
                        #         count = count + 1
                        # if count > 1:
                        #     y -= vy * (1/obj_dist) * 3
                        #     print('Continuous Turn in Y axis !!')
                        # else:
                        #     y += vy * (1/obj_dist) * 2

                        # if len(vy_list) > 1:
                        #     # print('vy_list length:',vy_list)
                        #     vy_list.pop(0)

                        
                        # if abs(vx) > 0.3:
                        #     # print('vx: ', np.round(vx, 2))
                        #     # print('obj_dist: ',obj_dist)
                        #     x += vx * (1/obj_dist)

                        # if x < 3:
                        #     print('vy: ', np.round(vy, 2))
                        #     print('obj_dist: ',obj_dist)
                        #     x += vx * 0.35
                        #     y += vy * 0.35
                            # y += vy * (1/obj_dist)
                    ...
                    # z = np.mean(np.array(sot_list[-4:])[:, :, 2])
                    # x = np.mean(np.array(sot_list[-2:])[:, :, 0])
                    # y = np.mean(np.array(sot_list[-2:])[:, :, 1])
                    # z = np.mean(np.array(sot_list[-3:])[:, :, 2])
                    if obj_dist <3:
                        box_z_filter = -0.3
                        near_flag = True
                    else:
                    ...
                    # print('temp_pc: ',temp_pc.shape[0])
                    # print('obj_dist: ',np.round(obj_dist,2))
                        
                    # print("obj_dist,  box_z_filter : {} {}".format(obj_dist, box_z_filter))
                    # if len(sot_list) > 2:
                    #     w, l, h = sot_list[-1][0][3:6]
                    #     volume = w * l * h
                    #     prev_w, prev_l, prev_h = sot_list[-2][0][3:6]
                    #     prev_volume = prev_w * prev_l * prev_h
                    #     volume_change = volume/prev_volume
                        # if volume_change > 1.35:
                        #     print('volume, volume_change: ', np.round(volume, 2), np.round(volume_change, 2))
                    ...
                    if ocl_count < args.max_occlusion:
                        try:
                            ...
                            cut_range = np.array([x-0.36, y-0.36, z-0.2, x+0.36, y+0.36, z+0.2])
                            temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                            # print('max z: ', np.max(temp_pc[:,2]))
                            # print('min z: ', np.min(temp_pc[:,2]))
                            # print('estimated height: ', np.max(temp_pc[:,2])-np.min(temp_pc[:,2]))
                            # import pdb; pdb.set_trace()
                            ...
                            # temp_pc = remove_ground_points(temp_pc)
                            # import pdb; pdb.set_trace()
                            # clusters = cluster_obstacles_2(temp_pc, eps=0.1, min_samples=10)
                            clusters = cluster_obstacles_2(temp_pc, eps=0.1, min_samples=15)
                            ocl_count = 0
                            ...
                            # bbox_list = bbox_list[(bbox_list[:, 2]<1.45), :] # z upper bound
                            # bbox_list = bbox_list[(bbox_list[:, 2]<1.5), :] # z upper bound
                            ...
                            z = np.mean(np.array(sot_list[-3:])[:, :, 2])
                            # ocl_count += 1
                            # if z > 0.8 and (sot_list[-1][0][2]/sot_list[-2][0][2] > 1.1):
                            #     print("z change huge ~~: ", sot_list[-1][0][2]/sot_list[-2][0][2])

                            print("fully occlusion! : {}, {}".format(ocl_count, args.max_occlusion))
                            try:
                                ## if z is too high, get the previous frame
                                    # x, y, z = sot_list[-3][0][:3]
                                    # x, y, z = sot_list[-4][0][:3]
                                    # x, y, z = sot_list[-5][0][:3]
                                # cut_range = np.array([x-0.85, y-0.85, z-0.4, x+0.85, y+0.85, z+0.5])
                                cut_range = np.array([x-1., y-1., z-0.4, x+1., y+1., z+0.5])
                                # cut_range = np.array([x-1., y-1., z-0.5, x+1., y+1., z+0.4])
                                temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                                ...
                                # clusters = cluster_obstacles_2(temp_pc, eps=0.2, min_samples=5, max_points=150)
                                clusters = cluster_obstacles_2(temp_pc, eps=0.3, min_samples=5)
                                # clusters = cluster_obstacles_2(temp_pc, eps=0.3, min_samples=15)
                                # print(clusters[-1])
                                bbox_list = calculate_bounding_boxes(clusters)
        ...
        if delay > 350:
            print("center : {} {} {}".format(np.round(x,2),np.round(y,2),np.round(z,2)))
            print("vel : {} {} {}".format(np.round(vx,2),np.round(vy,2),np.round(vz,2)))
...
if __name__ == '__main__':
    ...
    model_2d = torch.hub.load("ultralytics/yolov5", "custom", "/mnt/hanhwa/yolov5/yolov5s")
