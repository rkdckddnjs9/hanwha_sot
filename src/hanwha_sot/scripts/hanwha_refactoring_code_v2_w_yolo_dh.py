...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        global volume, volume_flag, max_volume_bbox, bbox_list_stack
        # start = time.time()
                ...
                if slc_label == 1:
                    ...
                    if ocl_count < args.max_occlusion:
                        try:
                            ...
                            if temp_pc.shape[0] < 100:
                                max_l, max_w, max_h = np.max(np.array(sot_list)[:, :, 3]), np.max(np.array(sot_list)[:, :, 4]), np.max(np.array(sot_list)[:, :, 5])
                                cut_range_2 = np.array([x-max_w/4, y-max_l/4, z-max_h/4, x+max_w/4, y+max_l/4, z+max_h/4])
                            ...
                            try:
                                ...
                                if temp_pc.shape[0]<150 and 0.01<volume <0.025 and volume_flag ==0 and x > 2 : #for temporary occlusion
                                     #import pdb; pdb.set_trace()
                                     if z-0.33 > 0.15:
                                        append = [x,y,z-0.33]+list(max_volume_bbox[3:6]) 
                                     else :  
                                        append =[x,y,z-0.05] +list(max_volume_bbox[3:6])                         
                                     append.append(0)
                                     bbox_list_stack = np.array(append)
                                     bbox_list_stack= bbox_list_stack.reshape(1, -1)
                                     bbox_list = bbox_list_stack
                                    
                                     volume_flag+=1
                                else:
                                    if volume_flag >=1 and volume_flag <5 :
                                        new=[x,y]
                                        bbox_list_stack[0][:2]= new
                                        bbox_list = bbox_list_stack
                                        volume_flag +=1
                                ...
                                #add bbox_processing
                                volume = temp_[0]['pred_boxes'][0][3]*temp_[0]['pred_boxes'][0][4]*temp_[0]['pred_boxes'][0][5]
                                if len(prior_bbox_list) < 12:
                                    prior_bbox_list.append(temp_[0]['pred_boxes'][0][0:6])
                                    volume_list.append(volume)
                                else:
                                    prior_bbox_list.append(temp_[0]['pred_boxes'][0][0:6])
                                    prior_bbox_list.pop(0)
                                    volume_list.append(volume)
                                    volume_list.pop(0)
                                max_volume_index = volume_list.index(max(volume_list))
                                max_volume_bbox = prior_bbox_list[max_volume_index]
...
if __name__ == '__main__':
    ...
    model_2d = torch.hub.load("ultralytics/yolov5", "custom", args.yolo_pt)
