...
def transform_3d_to_2d(box3ds):
    ...
    for idx, box3d in enumerate(box3ds):
        ...
        # horizontal_indices = horizontal_indices - 512
...
def corners_to_rv_pixels(corners, fov_up=22.5, fov_down=-22.5, H=128, W=1024):
    """
    검출된 객체의 코너점들을 RV 이미지 상의 픽셀 좌표로 변환하고,
    각 객체에 대한 (xmin, ymin, xmax, ymax)을 계산합니다.

    Args:
        corners (np.array): (M, 8, 3) 형태의 코너점 좌표.
        fov_up (float): LiDAR의 최대 고도각 (도).
        fov_down (float): LiDAR의 최소 고도각 (도).
        H (int): RV 이미지의 높이(수직 해상도).
        W (int): RV 이미지의 너비(수평 해상도).

    Returns:
        np.array: 각 객체에 대응되는 RV 이미지 상의 (xmin, ymin, xmax, ymax) 좌표 (M, 4).
    """
    # 코너점의 거리, 고도각, 방위각 계산
    ...
    # xmin = 1023 - np.max(pixel_x, axis=1)
    # ymin = np.min(pixel_y, axis=1)
    # xmax = 1023 - np.min(pixel_x, axis=1)
    # ymax = np.max(pixel_y, axis=1)
