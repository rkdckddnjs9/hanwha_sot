...
class ROS_MODULE:
    def __init__(self):
        ...
        self.pointcloud_pub = rospy.Publisher(
                '/arion/mtt/core/points_det', PointCloud2, queue_size=10
                ) 
        self.gtbox_array_pub = rospy.Publisher(
                '/arion/mtt/core/detect_gtbox', MarkerArray, queue_size=10
                )
        self.marker_pub = rospy.Publisher(
                '/arion/mtt/core/detect_box3d', MarkerArray, queue_size=10
                )

        self.marker_text_pub = rospy.Publisher(
                '/arion/mtt/core/text_det', MarkerArray, queue_size=10
                )
    ...
    @staticmethod
    def gpu2cpu_w_id(data_dict, pred_dicts):
        data_dict['points'] = data_dict['points'].cpu().numpy()
        if 'gt_boxes' in data_dict:
            data_dict['gt_boxes'] = data_dict['gt_boxes'].cpu().numpy()
        pred_dicts[0]['pred_boxes'] = pred_dicts[0]['pred_boxes'].cpu().numpy()
        pred_dicts[0]['pred_scores'] = pred_dicts[0]['pred_scores'].cpu().numpy()
        pred_dicts[0]['pred_labels'] = pred_dicts[0]['pred_labels'].cpu().numpy()
        pred_dicts[0]['pred_id'] = pred_dicts[0]['pred_id'].cpu().numpy()
        torch.cuda.empty_cache()
        return data_dict, pred_dicts
 
    def ros_print(self, pts, pred_dicts=None, last_box_num=None, gt_boxes=None, last_gtbox_num=None, class_id=None):
        ...
        if pred_dicts is not None:
            ...
            for obid in range(boxes.shape[0]):
                ...
                markert.pose.position.z = (boxes[obid][0][2] + 2*boxes[obid][4][2])
                markert.text = self.class_names[label[obid]-1] + ':' + str(np.floor(score[obid] * 100)/100) + ':' + str(class_id[obid])
    ...
    def ros_print_wo_id(self, pts, pred_dicts=None, last_box_num=None, gt_boxes=None, last_gtbox_num=None):
        def xyzr_to_pc2(pts, stamp, frame_id):
            data = np.zeros(pts.shape[0], dtype=[('x', np.float32), ('y', np.float32), ('z', np.float32), ('intensity', np.float32)])
            data['x'] = pts[:, 0]
            data['y'] = pts[:, 1]
