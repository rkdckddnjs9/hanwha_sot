...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        pred_dicts = self.transform_track_np(outputs)
        ...
        if skip_flag:
            ...
            if selected_id == None:
                ...
                try: # target object in detection result
                    ...
                    # for clustering area transform
                    x,y,z = sot_list[-1][0][:3]
                    # local_center = np.array([x,y,z,1])
                    # global_center = (np.linalg.inv(earth2os_sensor) @ local_center.T).T
                    local_center = np.array([x,y,z,1])
                    cut_range = np.array([x-2, y-2, z-0.7, x+2, y+2, z+0.7])
                    ...
                    # temp_[0]['pred_labels'] = np.full(bbox_list.shape[0], slc_label)
