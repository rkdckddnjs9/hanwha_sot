...
def tf_callback_3(data):
    ...
    # for global to os_sensor : earth -> odom -> base_link -> os_sensor
    ...
    earth2os_sensor = odom2base @ imu2os_sensor @ sensor2lidar
...
def pub_target_info(data, global_box):
    ...
    # custom_msg.class_ = Int8(label2pub[index2label[data['pred_labels'].item()]])
    custom_msg.object_class = Int8(label2pub[index2label[data['pred_labels'].item()]])
...
def pub_candidate_info(data, global_box):
    ...
    custom_msg.class_1 = Int8(label2pub[index2label[data['pred_labels'][0].item()]])
    ...
    custom_msg.class_2 = Int8(label2pub[index2label[data['pred_labels'][1].item()]])
    ...
    custom_msg.class_3 = Int8(label2pub[index2label[data['pred_labels'][2].item()]])
    ...
    custom_msg.class_4 = Int8(label2pub[index2label[data['pred_labels'][3].item()]])
    ...
    custom_msg.class_5 = Int8(label2pub[index2label[data['pred_labels'][4].item()]])
...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            if pred_dicts[0]['pred_labels'].shape[0] != 0:
                ...
                if _pred_dicts_[0]['pred_boxes'].shape[0] >= args.num_candidate:
                    ...
                    temp_label = np.ones((5,), dtype=np.int32)
            ...
            homogeneous_center[:3] = rotated_box[:3]
            # homogeneous_center = (np.linalg.inv(earth2os_sensor.copy()) @ homogeneous_center.T).T
            homogeneous_center = (np.linalg.inv(imu2os_sensor @ sensor2lidar) @ homogeneous_center.T).T # lidar -> base_link
            homogeneous_center = (odom2base @ homogeneous_center.T).T # base_link -> odom
            # homogeneous_center = (np.linalg.inv(odom2base) @ homogeneous_center.T).T # base_link -> odom
            # print("homogeneous_center {}".format(homogeneous_center))
            rotated_box[:3] = homogeneous_center[:3]
            # print("rotated_box {}".format(rotated_box[:3]))
            # odom2base @ imu2os_sensor @ sensor2lidar
            # print("sensor2lidar {}".format(sensor2lidar[:3, :3]))
        ...
        # global last_box_num
        if pc_viz == False:
            if selected_id != None and slc_label != 1:
                if ocl_count ==0:
                    # 임시 박스 h 변경 
