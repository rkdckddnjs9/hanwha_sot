...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if slc_label == 1:   #차량 처리 및 전이
                ...
                if ocl_count < args.max_occlusion:
                    try:
                        ...
                        # bias_direction, bias_magnitude = calculate_point_cloud_bias(box=temp_[0]['pred_boxes'][0][:3], points=temp_pc)
                        # print(bias_direction, bias_magnitude)
                        # import pdb; pdb.set_trace()
                        ...
                        try:
                            ...
                            # # bias_direction, bias_magnitude = calculate_point_cloud_bias(box=temp_[0]['pred_boxes'][0][:3], points=temp_pc)
                            # bias_direction, bias_magnitude = calculate_point_cloud_bias(box=[x, y, z], points=temp_pc)
                            # print(bias_direction, bias_magnitude)
            ...
            # print(_pred_dicts_[0]['pred_boxes'])
            # print("box_z_filter : {} \n".format(box_z_filter))
            # print("vx, vy, var_yaw : {} {} {}\n".format(vx, vy, var_yaw))
            # bias_direction, bias_magnitude = calculate_point_cloud_bias(box=temp_[0]['pred_boxes'][0][:3], points=temp_pc)
            # print("bias_direction, bias_magnitude {} {} \n".format(bias_direction, bias_magnitude))
        ...
        green_color = (0,255,0)
        box3dto2d = transform_3d_to_2d(box3ds=_pred_dicts_[0]['pred_boxes'])
        for box in box3dto2d:
            box_ = box.astype(int)
            cv2.rectangle(nearir_img_, tuple(box_[:2].tolist()), tuple(box_[2:].tolist()), green_color)
        for_pub_img = self.numpy_to_imgmsg(nearir_img_)
...
if __name__ == '__main__':
    ...
    target_info_pub = rospy.Publisher('FollowTargetInfo', FollowTargetInfo, queue_size=10)
    candidate_info_pub = rospy.Publisher('TargetCandidate', TargetCandidate, queue_size=10)
