...
from vis_ros_ori import ROS_MODULE
ros_vis = ROS_MODULE()
last_box_num = 0
last_gtbox_num = 0
...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        mask_indices = pred_dicts[0]['pred_scores'].argsort()[::-1]
        pred_dicts[0]['pred_boxes'] = pred_dicts[0]['pred_boxes'][mask_indices]
        pred_dicts[0]['pred_scores'] = pred_dicts[0]['pred_scores'][mask_indices]
        pred_dicts[0]['pred_labels'] = pred_dicts[0]['pred_labels'][mask_indices]
        # pred_dicts[0]['pred_id'] = pred_dicts[0]['pred_id'][mask_indices]
        
        mask_ped = np.where(pred_dicts[0]['pred_labels']==2)[0][:10]
        mask_car = np.where(pred_dicts[0]['pred_labels']==1)[0][:10]
        mask = np.zeros_like(mask_indices, dtype=bool)
        mask[mask_ped] = True
        mask[mask_car] = True
        ...
        # pred_dicts[0]['pred_id'] = pred_dicts[0]['pred_id'][mask]
        # pdb.set_trace()
