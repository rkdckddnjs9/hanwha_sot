...
class ros_demo():
    ...
    def online_inference(self, msg):
        global selected_id, _pred_dicts_, tracklet_nums, PCs, temp_selected_id, slc_label, sot_list, count, mask_dist
        ...
        if selected_id == None:
            ...
            if count <= 10 :
                ...
                # pdb.set_trace()
                pre_BB = _pred_dicts_[0]['pred_boxes']
                mask_dist = self.get_target_distance(pre_BB, pred_dicts_[0]['pred_boxes'])
                # mask_dist = self.get_target_distance_v2(pre_BB, pred_dicts_[0]['pred_boxes'])[0]
                mask_dist = mask_dist.argsort()[::-1]
                mask = np.zeros_like(mask_dist, dtype=bool)
                mask[mask_dist[-1]] = True
                
                slc_label_ = (pred_dicts_[0]['pred_labels']==slc_label)
                # mask_dist = mask_dist*slc_label_
                mask_dist = mask*slc_label_
                # print(mask_dist)
