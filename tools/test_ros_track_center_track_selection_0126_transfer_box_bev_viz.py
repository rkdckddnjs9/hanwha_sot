...
def pc2bev(pc):
    ...
    # pdb.set_trace()
    # BEV 이미지의 중심을 기준으로 좌표 재조정
    x_pixels = np.int32(bev_h / 2 - x_pixels + 500)
    y_pixels = np.int32(bev_w / 2 - y_pixels + 500)
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if vis_bev:
                ...
                # feat viz
                # # pdb.set_trace()
                # bev_feat = data_infer['spatial_features_2d'][0].cpu().detach().numpy() # [128, 448, 288]
                # # bev_feat = (np.max(np.transpose(bev_feat, (1,2,0)), axis=2)*255).astype("uint8")
                # bev_feat = ((np.max(np.transpose(bev_feat, (2,1,0)), axis=2))*255).astype("uint8")
                # bev_feat = cv2.applyColorMap(bev_feat,cv2.COLORMAP_JET)
                
                # pdb.set_trace()
                # for box in pred_dicts_[0]['pred_boxes']:
                #     # self.point_cloud_range = [0, -44.8, -2, 57.6, 44.8, 4]
                #     min_x, min_y, max_x, max_y = get_2d_bbox_from_3d(box, 448, 224, [0, 57.6], [-44.8, 44.8])
                #     # min_x += 112
                #     # max_x += 112
                #     # min_y += 224
                #     # max_y += 224
                #     cv2.rectangle(bev_feat, (min_x, min_y), (max_x, max_y), (0, 255, 0), 2)
                # cv2.imshow("bev_feat", bev_feat)
                # cv2.waitKey(1)
                # pdb.set_trace()
                
                
                ros_image = self.bridge.cv2_to_imgmsg(bev_feat[400:, :], "bgr8")
