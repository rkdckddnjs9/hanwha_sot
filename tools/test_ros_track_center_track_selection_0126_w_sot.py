        ...
        if selected_id == None:
            ...
            if tracklet_nums == 0:
                ...
                if best_match_index == None:
                    _pred_dicts_[0]['pred_boxes'] = np.concatenate([box.center[np.newaxis, ...], box.wlh[np.newaxis, ...][:, [1,0,2]], np.array(quaternion_to_yaw(box.orientation.q)).reshape(1, 1)], axis=1)
                    _pred_dicts_[0]['pred_scores'] = _pred_dicts_[0]['pred_scores']
                    _pred_dicts_[0]['pred_labels'] = _pred_dicts_[0]['pred_labels']
                    _pred_dicts_[0]['pred_id'] = _pred_dicts_[0]['pred_id']
                else:
                    # pdb.set_trace()
                    _pred_dicts_[0]['pred_boxes'] = pred_dicts[0]['pred_boxes'][best_match_index].reshape(1, -1)
                    _pred_dicts_[0]['pred_scores'] = pred_dicts[0]['pred_scores'][best_match_index].reshape(-1)
                    _pred_dicts_[0]['pred_labels'] = pred_dicts[0]['pred_labels'][best_match_index].reshape(-1)
                    _pred_dicts_[0]['pred_id'] = _pred_dicts_[0]['pred_id']
                pass
...
if __name__ == '__main__':
    ...
    # =========== for SOT tracking =====================================
    global count, tracklet_nums, results_BBs, PCs, sot_list, index2label
