...
def transform_pc2range(pc):
    ...
    # range_view_image = np.zeros((num_vertical_samples, num_horizontal_samples), dtype=np.uint32)
    ...
    # range_view_image[vertical_indices[valid_indices], horizontal_indices[valid_indices]] = normalized_depth[valid_indices].astype(np.uint32)
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if vis_range:
                ...
                test_ = cv2.applyColorMap(range_view_image,cv2.COLORMAP_COOL)
                # test_ = range_view_image.copy()
                ...
                # ros_image = self.bridge.cv2_to_imgmsg(test_, "16UC1")
