...
def range_img_callback(data):
    global img_sub
    # pdb.set_trace()
    # img_sub = CvBridge().imgmsg_to_cv2(data, "32FC1")
    img_sub = CvBridge().imgmsg_to_cv2(data, "8UC1")
    img_sub = cv2.flip(img_sub, 0)
    img_sub = cv2.normalize(img_sub, None, 0, 255, cv2.NORM_MINMAX)
    return img_sub

def rec_img_callback(data):
    global img_sub
    # pdb.set_trace()
    # img_sub = CvBridge().imgmsg_to_cv2(data, "32FC1")
    img_sub = CvBridge().imgmsg_to_cv2(data, "mono8")
    img_sub = cv2.flip(img_sub, 0)
    # img_sub = cv2.normalize(img_sub, None, 0, 255, cv2.NORM_MINMAX)
    return img_sub
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if vis_range:
                ...
                # range_view_image = img_sub
                # pdb.set_trace()
                test_ = img_sub #.astype("uint8")
                lt, rb = transform_3d_to_2d(box3d=_pred_dicts_[0]['pred_boxes'])
                # test_ = cv2.applyColorMap(range_view_image,cv2.COLORMAP_JET)
                # test_ = cv2.applyColorMap(range_view_image.astype("uint8"),cv2.COLORMAP_JET)
                
                # test_ = cv2.applyColorMap(test_,cv2.COLORMAP_JET)
                ...
                if kcf_count == 0:
                    ...
                    # cv2.rectangle(test_, lt, rb, 255, 4)
                else:
                    # pdb.set_trace()
                ...
                # cv2.rectangle(test_, lt, rb, 100, 1)
                ...
                ros_image = self.bridge.cv2_to_imgmsg(test_, "mono8")
...
if __name__ == '__main__':
    ...
    # img_sub = rospy.Subscriber("/ouster/range_image", Image, range_img_callback)
    # img_sub = rospy.Subscriber("/ouster/reflec_image", Image, range_img_callback)
    img_sub = rospy.Subscriber("/ouster/nearir_image", Image, range_img_callback)
