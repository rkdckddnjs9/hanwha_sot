...
def tf_callback_3(data):
    global count, earth2map, map2odom, odom2base, imu2os_sensor, sensor2lidar, earth2os_sensor, init_odom_rt, odom_flag, odom_list, lidar_z_list
    # for global to os_sensor : earth -> map -> odom -> base_link -> imu_link -> os_sensor -> os_lidar
    # base_link2imu_link : translation: [0,0,0], quaternion:[0,0,0,1]
    # imu_link2os_sensor
    ...
    for transform in data.transforms:
        ...
        # odom -> base_link
            ...
            if len(odom_list) < 4:
                odom_list.append(quaternion_to_yaw(odom2base_quaternion))
            else:
                odom_list.append(quaternion_to_yaw(odom2base_quaternion))
                odom_list.pop(0)
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if slc_label == 1:   #차량 처리 및 전이
                ...
                try:
                    try:
                        # cut_range = np.array([x-0.4, y-0.4, z-0.4, x+0.4, y+0.4, z+0.55])
                        cut_range = np.array([x-1, y-1, z-0.4, x+1, y+1, z+0.7])
                        # cut_range = np.array([x-2, y-2, z-0.4, x+2, y+2, z+0.7])
                        temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                        # temp_pc = mask_points_out_of_range_2(data_dict['points'][:, :3], cut_range)
                        clusters = cluster_obstacles_2(temp_pc)
                    except: # if no object # larger receptive field!
                        cut_range = np.array([x-1.5, y-1.5, z-0.4, x+1.5, y+1.5, z+0.7])
                        # cut_range = np.array([x-2, y-2, z-0.4, x+2, y+2, z+0.7])
                        temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                        # temp_pc = mask_points_out_of_range_2(data_dict['points'][:, :3], cut_range)
                        clusters = cluster_obstacles_2(temp_pc)
                    ...
                    print("occlusion !! : {}/{}".format(ocl_count, 20))
                    if ocl_count == 10:
                        import pdb; pdb.set_trace()
...
if __name__ == '__main__':
    ...
    # =========== for SOT tracking =====================================
    global count, tracklet_nums, results_BBs, sot_list, index2label, ocl_count, cluster_count, odom_list, lidar_z_list
    cluster_count=0
