...
def tf_callback_3(data):
    ...
    for transform in data.transforms:
        ...
        # odom -> base_link
            ...
            if len(odom_list) < 4:
                odom_list.append(quaternion_to_pitch(odom2base_quaternion))
            else:
                odom_list.append(quaternion_to_pitch(odom2base_quaternion))
                odom_list.pop(0)
...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        # prev_pitch = odom_list[-2]
        # cur_pitch = odom_list[-1]
        # var_pitch = prev_pitch - cur_pitch
        # rot_m = pitch_to_rotation_matrix(-var_pitch)
        # rot_m = pitch_to_rotation_matrix(var_pitch)
        # data_dict['points'][:, :3] = (rot_m @ data_dict['points'][:, :3].T).T
        
        # data_dict['points'][:, :3] = (odom2base[:3, :3] @ data_dict['points'][:, :3].T).T
        data_dict['points'][:, 2] += var_z
        # import pdb; pdb.set_trace()
