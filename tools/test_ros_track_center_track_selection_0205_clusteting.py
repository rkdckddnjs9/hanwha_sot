...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        # # pc_ = convert_ros_to_o3d(data_infer['points'][:, 1:])
        # object_ = cluster_obstacles_2(data_infer['points'][:, 1:])
        # bbox_list = []
        # for cluster in object_:
        #     # Calculate PCA-based oriented bounding box for each cluster
        #     center, size = calculate_aabb(cluster)
        #     bbox_list.append(np.concatenate([center[:3], size]))
        # bbox_list = np.vstack(bbox_list)
        
        temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], [10, -10, -2, 20, 10, 4])
        # clusters = euclidean_clustering(data_infer['points'][:, 1:4], tolerance=0.6, min_points=100)
