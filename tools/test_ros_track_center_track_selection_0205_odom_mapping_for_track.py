...
def cluster_obstacles_2(cloud, eps=2.0, min_samples=10, max_points=250):
...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        # local to global box
        local_center = pred_dicts[0]['pred_boxes'].clone().detach()
        local_center = torch.cat([local_center[:, :3], torch.ones((local_center.shape[0], 1)).to(local_center.device)], dim=1)
        # import pdb; pdb.set_trace()
        global_center = (torch.tensor(init_odom_rt).to(local_center.device).to(torch.float32) @ local_center.T).T
        pred_dicts[0]['pred_boxes'][:, :3] = global_center[:, :3]
        ...
        # local region - clsutering method
        # temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], [10, -10, -2, 20, 10, 4])
        # clusters = euclidean_clustering(temp_pc, tolerance=0.6, min_points=100)
        # bbox_list = calculate_bounding_boxes(clusters)
        # bbox_list = bbox_list[(bbox_list[:, 2]<1), :]
        
        # temp_ = pred_dicts.copy()
        # temp_[0]['pred_boxes'] = bbox_list
        # temp_[0]['pred_scores'] = np.ones(bbox_list.shape[0])
        # temp_[0]['pred_labels'] = np.ones(bbox_list.shape[0]).astype(np.int8)
        # temp_[0]['pred_id'] = np.ones(bbox_list.shape[0]).astype(np.int8)
        # global last_box_num
        ...
        if skip_flag:
            ...
            if selected_id == None:
                ...
                pose_pub.publish(transform_pose_stamped(sot_list[-1][:3]))
            ...
            # global to local
            global_center = _pred_dicts_[0]['pred_boxes'].copy()
            # import pdb; pdb.set_trace()
            global_center = np.concatenate([global_center[:, :3], np.ones(global_center.shape[0]).reshape(-1, 1)], axis=1)
            local_center = (np.linalg.inv(init_odom_rt) @ global_center.T).T
            _pred_dicts_[0]['pred_boxes'][:, :3] = local_center[:, :3]
