    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if slc_label == 1:   #차량 처리 및 전이
                ...
                # var_pitch = prev_pitch - cur_pitch
                rot_m = pitch_to_rotation_matrix(var_pitch)
                # rot_m = pitch_to_rotation_matrix(-var_pitch)
                # rot_m = pitch_to_rotation_matrix(-cur_pitch)
                x, y, z = rot_m @ np.array([x,y,z])
                ...
                try:
                    try:
                        # cut_range = np.array([x-0.7, y-0.7, z-0.4, x+0.7, y+0.7, z+0.5]) # good case
                        ...
                        try:
                            ...
                            # clusters = cluster_obstacles_2(temp_pc)
                            clusters = cluster_obstacles_2(temp_pc, eps=0.2, min_samples=5)
                            bbox_list = calculate_bounding_boxes(clusters)
        ...
        if pc_viz == False:
            if selected_id != None:
                # 임시 박스 h 변경 
                _pred_dicts_[0]['pred_boxes'][0, 5] = 1.6
