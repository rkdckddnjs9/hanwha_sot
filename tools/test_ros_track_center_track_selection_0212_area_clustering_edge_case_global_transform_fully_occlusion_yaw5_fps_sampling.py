...
import hdbscan
def hdbscan_cluster_obstacles_2(cloud, min_cluster_size=10, min_samples=1):
...
def remove_ground_points(points, n_degree=5, threshold=1):
    ...
    Args:
        points: (N, 3) ndarray of points.
        n_degree: Degree of the polynomial to fit.
        threshold: RANSAC threshold to consider a point as an inlier.

    Returns:
        non_ground_points: (M, 3) ndarray of non-ground points.
    """
...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            # cut_range = np.array([x-1, y-1, z-0.4, x+1, y+1, z+0.7])
            # cut_range = np.array([x-2, y-2, z-0.4, x+2, y+2, z+0.7])
            ...
            if slc_label == 1:   #차량 처리 및 전이
                ...
                if ocl_count >= 25:
                    ...
                    if slc_label_.sum() == 0: # no same cls obj in current detection result 
                        ...
                        mask_dist = self.get_target_distance(prev_obj, pred_dicts_mot[0]['pred_boxes'])
                ...
                # odom topic average hz : 50 Hz -> delta theta * 5
                prev_yaw = odom_yaw_list[-2]
                cur_yaw = odom_yaw_list[-1]
                var_yaw = cur_yaw - prev_yaw
                rot_m = yaw_to_rotation_matrix(var_yaw*5)
                ...
                rot_m = pitch_to_rotation_matrix(var_pitch*5)
                x, y, z = rot_m @ np.array([x,y,z])
                ...
                if ocl_count < 40:
                    try:
                        # cut_range = np.array([x-0.4, y-0.4, z-0.4, x+0.4, y+0.4, z+0.5])
                        cut_range = np.array([x-0.36, y-0.36, z-0.4, x+0.36, y+0.36, z+0.5])
                        temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                        # temp_pc = mask_points_out_of_range_2(data_dict['points'][:, :3], cut_range)
                        # clusters = cluster_obstacles_2(temp_pc)
                        if np.sqrt(np.array(x**2+y**2)) < 4:
                            # import pdb; pdb.set_trace()
                            start = time.time()
                        ...
                        if np.sqrt(np.array(x**2+y**2)) < 30 and np.sqrt(np.array(x**2+y**2)) > 10:
                            start = time.time()
                            temp_pc = remove_ground_points(temp_pc)
                        ...
                        # bbox_list = bbox_list[(bbox_list[:, 2]>0.), :]
                        # bbox_list = bbox_list[(bbox_list[:, 2]>-0.3), :]
                        # bbox_list[:, 5] = sot_list[-1][0][5]   # l,w,h =  (3,4,5)
                        # bbox_list[:, 5] = init_h
                        # import pdb; pdb.set_trace()
                        ...
                        try:
                            ...
                            #cut_range = np.array([x-1.0, y-1.0, z-0.4, x+1.0, y+1.0, z+0.5])
                            temp_pc = mask_points_out_of_range_2(data_infer['points'][:, 1:4], cut_range)
                            if np.sqrt(np.array(x**2+y**2)) < 4:
                                # import pdb; pdb.set_trace()
                                start = time.time()
                            ...
                            # tt_ = data_dict['points'][:, :3].copy()
                            # tt_[:, 2] -= var_z
                            # temp_pc = mask_points_out_of_range_2(tt_, cut_range)
                            # clusters = cluster_obstacles_2(temp_pc)
                            # import pdb; pdb.set_trace()
                            clusters = cluster_obstacles_2(temp_pc, eps=0.3, min_samples=5)
                            bbox_list = calculate_bounding_boxes(clusters)
                            # bbox_list = bbox_list[(bbox_list[:, 3]<1.8), :]
                            # bbox_list = bbox_list[(bbox_list[:, 4]<1.8), :]
                            bbox_list = bbox_list[(bbox_list[:, 2]<1.3), :] #center_z filtering
                            bbox_list = bbox_list[(bbox_list[:, 3]<1.3), :] # l filtering
                            bbox_list = bbox_list[(bbox_list[:, 4]<1.3), :] # w filtering
                            bbox_list = bbox_list[(bbox_list[:, 5]>0.1), :] # h filtering
                            bbox_list = bbox_list[(bbox_list[:, 2]>-0.3), :]
                            # bbox_list = bbox_list[(bbox_list[:, 2]>0.), :]
                            # bbox_list = bbox_list[(bbox_list[:, 2]>z-0.2), :]
                            bbox_list = bbox_list[nms_without_scores(bbox_list, 0.9)[:10]]
                            # bbox_list[:, 2] += var_z
                            dist_mask =  self.get_target_distance(sot_list[-1].copy(), bbox_list)
                            temp_ = [{}]
                    ...
                    print("occlusion !! : {}/{}".format(ocl_count, 40))
                    ...
                    try: # if no object in current frame
                        ...
                        if label_mask.sum() != 0:
                            ...
                            dist_mask =  self.get_target_distance_v2(sot_list[-1].copy(), pred_dicts[0]['pred_boxes'])
                            min_dist_idx = np.where(dist_mask[0]==dist_mask.min())[0].item()
                            dist_mask = dist_mask[0] < 5
