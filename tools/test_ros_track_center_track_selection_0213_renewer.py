...
def tf_callback_3(data):
    ...
    for transform in data.transforms:
        ...
        # odom -> base_link
            ...
            odom2base[:3, :3] = odom2base_rot
            odom2base[:3, 3] = odom2base_t    
            print("odom_z : {}".format(odom2base[3, 2]))
...
def remove_ground_points(points, n_degree=5, threshold=0.2):
    """
...
class ros_demo():
    ...
    def receive_from_ros_2(self, msg):
        pc = ros_numpy.numpify(msg)
        points_list = np.zeros((pc.shape[0], 4))
        points_list[:, 0] = copy.deepcopy(np.float32(pc['x']))
        points_list[:, 1] = copy.deepcopy(np.float32(pc['y']))
    ...
    def online_inference(self, msg):
        ...
        points_list = self.receive_from_ros_2(msg)
        
        input_pc = points_list[:, :3].copy()
        ...
        rot_m = init_odom_rt[:3, :3]
        input_pc = (rot_m@input_pc.T).T
        
        points_list[:, :3] = input_pc
        points_list = mask_points_out_of_range_2(points_list, self.point_cloud_range)
                ...
                if ocl_count < 40:
                    ...
                    try: # if no object in current frame
                        ...
                        if label_mask.sum() != 0:
                            ...
                            dist_mask =  self.get_target_distance(sot_list[-1].copy(), pred_dicts[0]['pred_boxes'])
                            temp_ = [{}]
