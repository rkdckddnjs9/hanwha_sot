...
class ros_demo():
    ...
    def online_inference(self, msg):
        global selected_id, _pred_dicts_, tracklet_nums, PCs, temp_selected_id, slc_label
        ...
        if selected_id == None:
            ...
            try:
                ...
                # pdb.set_trace()
                box3d = pred_dicts_[0]['pred_boxes'][mask_dist][0]
                score = pred_dicts_[0]['pred_scores'][mask_dist][0]
                label = pred_dicts_[0]['pred_labels'][mask_dist][0]
                _id = pred_dicts_[0]['pred_id'][mask_dist][0]
                slc_inst = BoundingBox(
                                        center=box3d[:3], #xyz
                                        size=box3d[3:6][[1,0,2]], #lwh->wlh
                                        orientation=Quaternion(heading_to_quaternion(box3d[-1])) , #yaw -> quaternion
