    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            try:
                ...
                box3d = pred_dicts_[0]['pred_boxes'][slc_idx]
                score = pred_dicts_[0]['pred_scores'][slc_idx]
                label = pred_dicts_[0]['pred_labels'][slc_idx]
                _id = pred_dicts_[0]['pred_id'][slc_idx]
                ...
                ppre_BB = results_BBs[-2]
                pre_BB = results_BBs[-1]

                # pdb.set_trace()
                vx, vy, vz = (pre_BB.center - ppre_BB.center)/2
                # pdb.set_trace()
                itp_box = BoundingBox(
                                        center=pre_BB.center + np.array([vx, vy, vz]), #xyz
                                        size=pre_BB.wlh, #lwh->wlh
                                        orientation=pre_BB.orientation , #yaw -> quaternion
                                        label=pre_BB.label,
                                        score=pre_BB.score,
                                        )
                results_BBs.append(itp_box)
                ...
                model_PC = kitti_utils.getModel([PCs[0], cur_PC], [results_BBs[0], slc_inst], scale=1.25)
