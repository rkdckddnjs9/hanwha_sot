...
class ros_demo():
    ...
    def online_inference(self, msg):
        ...
        #  score thresholding ====================================================
        mask_ped = ((dets_all['info'][:, 0] >0.3) * (dets_all['info'][:, 1] ==2.)) # ped
        mask_car = ((dets_all['info'][:, 0] >0.4) * (dets_all['info'][:, 1] ==1.)) # car
        ...
        if selected_id == None:
            ...
            # _, _ = sub_tracker.track(dets_all, frame_id, 'hanwha') #for SOT
            sub_tracker.copy_state_to(tracker)
            # sub_tracker = tracker
            # pdb.set_trace()
            ...
            try:
                ...
                cur_box = seq_list[-1][0]['pred_boxes'][0][np.newaxis, ...]
                vel = (cur_box[0][:3] - seq_list[-2][0]['pred_boxes'][0][:3])/2
                cur_box[0][:3] = cur_box[0][:3] + vel
                info = np.concatenate([seq_list[-1][0]['pred_scores'].reshape(1, -1), seq_list[-1][0]['pred_labels'].reshape(1, -1)], axis=1)
                try:
                    dets_all['dets'] = np.concatenate([dets_all['dets'], cur_box], axis=0)
                    dets_all['info'] = np.concatenate([dets_all['info'], info], axis=0)
                except:
                    pdb.set_trace()
                    
                
                results, affi = sub_tracker.track(dets_all, frame_id, 'hanwha')
                tracker.copy_state_to(sub_tracker)
                ...
                try:
                    slc_idx = np.where(np.array(results[0][:, 7], dtype=np.int8)==selected_id)[0][0]
                except:
                    pdb.set_trace()
                pred_dicts_[0]['pred_boxes'] = _pred_dicts_[0]['pred_boxes'][slc_idx][np.newaxis, ...]
                pred_dicts_[0]['pred_scores'] = _pred_dicts_[0]['pred_scores'][slc_idx][np.newaxis, ...]
