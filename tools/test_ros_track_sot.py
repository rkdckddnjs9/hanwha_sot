    ...
    def online_inference(self, msg):
        ...
        if selected_id == None:
            ...
            if tracklet_nums == 0:
                ...
                # print(slc_inst.orientation.radians)
                # print(box3d[-1])
                ...
                # model_PC = kitti_utils.getModel([PCs, cur_PC], [results_BBs[0], results_BBs[tracklet_nums-1]], scale=1.25)
                ...
                pred_dicts_[0]['pred_boxes'] = np.concatenate([box.center[np.newaxis, ...], box.wlh[np.newaxis, ...][:, [1,0,2]], np.array(box3d[-1]).reshape(1, 1)], axis=1)
